...
const [start,setStart]=useState();
  ...
  useEffect(() => {
    const onScroll = () => {
      ...
      setEnd(document.querySelector('.gall-cont').scrollHeight+document.querySelector('.gall-cont').clientHeight+600);
      setScrollHeight(window.scrollY);
  ...
  const style = {
    ...
    background: windowWidth<=1500?(scrollHeight >= start && scrollHeight <=end ? 'linear-gradient(#00284900, #002849)' : 'transparent'):(scrollHeight >= start && scrollHeight <=(end-800) ? 'linear-gradient(#00284900, #002849)' : 'transparent'),
